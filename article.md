...
However, as it turns out, these are generally impossible to compute; even something as simple as thw $n$-spheres are not fully understood thus severely limiting their use in topology.

Herein comes homology to help save the day.
Instead of discussing loops up to homotopy, we instead discuss loops up to homology, which is a slightly weaker (and much more difficult to explain) notion of equivalence.
...
However, while these tools offer valuable analytical insight into such spaces, they often fall short in cultivating an intuitive grasp for these topological concepts.
...
We will begin our investigation of topological spaces by understanding how we can generalize geometry and the notion of space with a metric space.
...
For this reason, we can define a more general concept of topology by only considering only the collection of subsets satisfying some properties, which we call basic open sets.
Then by forming the arbitrary union of these basic open sets, we obtain an topology.
Of course, we can also skip this step of forming basic open steps and give the definition of an open set to start.
With every thing we've talked about, we finally arive at the modern definition of a topological space, however a full understanding is not necessary for further discussion.
...
This is often seen as the perfered method of defining continuity on any topological space.*There are many reasons for why thinking about topology with respect to open sets is more useful than thinking about topology with respect to basic open sets.
...
Since closed sets in topology is defined to be the complement of an open set, they can in turn be interpreted as sets <em>with</em> boundary.*{:.foot-tag}

In topology, we will only consider maps for which are continuous, and in particular, when a map is continuous and a bijection, if the inverse of this map is continuous, then we say this map is an homeomorphism.*Note that there exists continuous bijections with a discontinuous inverses.*{:.foot-tag}
When there is a homeomorphism between two space, we say they are homeomorphic, which is to say these topological spaces are essentially equivalent (up to a relabeling of the points).

A slightly weaker notion of equivilence that is often useful is *homotopy*.
In particular, we are interested with the notion of homotopic functions.
...
{:.no-indent}
The next image is example of loops which aren't homotopic.

<div style="display: flex; justify-content: center; padding: 20px 0px">
    <img src="{{site.baseurl}}/assets/images/torus_gen.webp" alt="torus nonhomotopic loops" width="360px">
</div>

We can also define a slightly weaker notion of equivilence on spaces namely *homotopy equivilence*.
Two spaces are homotopy equvilent when there's some continuous deformation between them.
Formally, we require
...
For example, a point and a disk are evidently not homemomorphic, since their underlying sets have different cardinality.
They are in fact homotopy equivilent as one can crush a disk to a point continuously.
However, a circle is not homotopy equivilent to a point since no matter how much one deforms a circle, it will always have a "hole" in the middle.

## Manifolds
A topological space is an incredibly powerful definition which is often too broad for application in geometry.
Thus, we often restrict ourselves to a more selective class of well behaved topological space known as manifolds.
...
For any point, if we take sufficently small disk around it then deform it in a way to remove the curvature, we find that this set is homeomorphic to any open disk in the plane.
...
Other common examples of manifolds are shown below.*Technically the Mobius strip is a <em>manifold with boundary</em> since any neighborhood of a boundary point is hoemomorphic to the half plane. We will not fuss with the technicals in this articles, thus we will ignore this technicality.*{:.foot-tag}
...
Then, if we identify the two points where the ray from the origin intersects the two sphere, we get $S^2 \times S^1$.
...
To harbar a deeper understanding, we visulize a loop in $S^2 \times S^1$ by our representation of the space in the image below:
...
This time, we will start with $D^3$ and instead we will instead identify the antipodal points on the boundary.
...
Now with most things in mathematics, I could just opt to just give the definition of a group and whilst the definition is not too difficult to understand, at first glance, the motivation behind this construction may be difficult to see.
...
The act of walking to school from home is invertible since you can always just walk back home.*The act of walking to the gym from school shall be left uninvertible since the school disallows smelly students from entering the premise.*{:.foot-tag}
The first thing we should notice is that the set of possible actions someone can take in this situation is fully dependent on the location, or the vertices, and that taking any two actions, we can combine to be a new action.
...
These groups are known as the higher homotopy groups and they generalize the fundamental group to higher dimensional spaces, but this added complexity which makes it difficult to compute which hinders its usefulness.
...
Only until the end of the 19th century would a modern form of homology take shape by the hands of Poincaré.
...
We can generalize this to further dimensions by consider cycles of higher dimension, however, this is where we will end our discussion to avoid getting too technical.
...
It should be noted here that fiber bundles are not generally unique, and any triple consisting of a base space, a total space, and a standard fiber can give rise to enumerous set of fiberings, which is what our proceeding sections will address.
...
We won't go too much into detail about how this is done, but loosely speaking, we can quantify closeness between two fiberings by observing how big of a transformation we need to perform to get from one to another.

For any space $X$, if we let $\Omega X$ represent the space of loops arising from sme fixed basedpoint, we have the following theorem as shown in [WY24]:
...
The immediate result of this observation is that $H_n(SF^N)$ and $H_n(SF^T)$ are the infinite cyclic group for when $n$ is even, hence they generated by one element.
